...
def build(d):
    ...
    default_h1 = f"Hey {d['first']} — you're hiring <em>{d['eng_roles']} engineers</em>. How many will be tested for the way they'll actually work?"
    page = page.replace(
        "<h1>Hey David — you're hiring <em>43 engineers</em>. How many will be tested for the way they'll actually work?</h1>",
        f"<h1>{d.get('headline', default_h1)}</h1>")
    page = page.replace(
